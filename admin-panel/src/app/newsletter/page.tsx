    ...
    <div className="p-6 md:p-10 max-w-4xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <h1 className="text-2xl font-bold text-[#1C1C1E]" style={{ fontFamily: "'Cormorant Garamond', Georgia, serif" }}>
            Newsletter Subscribers
          </h1>
          <p className="text-sm text-slate-500 mt-1">{subscribers.length} total subscriber{subscribers.length !== 1 ? "s" : ""}</p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white border border-[#E0DDD6] shadow-sm rounded-2xl">
          <Mail size={40} className="text-slate-400 mx-auto mb-3" />
          <p className="text-slate-500 font-semibold">No subscribers yet.</p>
          <p className="text-slate-400 text-xs mt-1">Subscribers will appear here when customers sign up from the website footer.</p>
        </div>
      ) : (
        <div className="bg-white border border-[#E0DDD6] shadow-sm rounded-2xl overflow-hidden">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {subscribers.map((sub, index) => (
                  <tr key={sub.id} className="border-b border-[#E0DDD6] hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[#1C1C1E] font-medium">{sub.email}</span>
                      </div>
                    </td>
